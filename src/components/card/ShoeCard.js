...
function ShoeCard(data) {
    ...
    return(
        <div className="flex flex-col mt-4 space-y-2 px-4 w-auto md:w-auto lg:my-4 lg:px-4 lg:w-full hover:scale-110  cursor-pointer transition-all">
                {isDiscount && <Badge.Ribbon text={`-${discount}%`} color='#1fa7d5'></Badge.Ribbon>}
                <Link href={`/list/${id}`}>
            ...
            <div className='font-bold text-lg tracking-normal flex items-start pb-8'> 
            <span id="price-box" className="flex flex-row space-x-2">
                {isDiscount && <p className="text-md line-through pt-1">{`$${prevPrice}`}</p>}
