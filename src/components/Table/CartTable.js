    ...
    return (
      <div className="overflow-x-auto">
        <table className="border-collapse border border-slate-300 w-3/4 mx-auto ">
          ...
          <tbody>
            {items.map((item) => {
              return <TableRow key={item.key} itemData={item} />;
            })}
          </tbody>
        </table>
      </div>
    ...
    return (
      <div className="overflow-x-auto h-2/5 flex flex-col justify-center align-middle border-collapse border border-slate-300 w-1/2 mx-auto p-6">
        ...
        <Link href="/list" className="mx-auto">
          <h3 className="text-slate-700 hover:text-slate-400 scale-110">
        ...
        {columns &&
          columns.map((column) => {
            ...
            return (
              <th
                key={key}
                className={`border border-slate-300 p-3 uppercase w-${width}`}
              >
                {dataField}
              </th>
            );
...
function TableRow({ itemData }) {
  ...
  const handleIncrease = async () => {
    ...
    setLoading(true);
    await callApi("api/updateCartItem", "POST", {
      id: get(itemData, "_id", null),
      quantity: quantity + 1,
    });
    setLoading(false);
  };
  const handleDecrease = async () => {
    if (quantity === 1) {
      setLoading(true);
      await callApi("api/deleteCartItem", "DELETE", {
        id: get(itemData, "_id", null),
      });
      setLoading(false);
    } else {
  ...
  return (
    <tr className="border-b-2 border-slate-30">
      ...
      <td className="align-middle text-center h-36 text-gray-500 w-48">
        ${price}
      </td>
      ...
      <td className="align-middle text-center h-32 text-gray-500 w-48">
