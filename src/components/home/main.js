...
export default function Main({ session }) {
    ...
    !isEmpty(kartData)
      ...
      : openNotificationWithIcon(
          ...
          emptyMessage.description +
            " " +
            (session ? get(session, "user.name", "dude") : "")
        );
  };
  const handleCancel = () => {
    setIsModalOpen(false);
  ...
  const editAttributes = (action, selectedId, quantity, basePrice) => {
    if (["changeQuantity"].includes(action)) {
      setKartData((kartState) =>
        kartState.map((item) =>
          item.id === selectedId
            ? { ...item, quantity, price: basePrice * quantity }
            : item
        )
      );
    } else if (["delete"].includes(action)) {
      setKartData((kartState) =>
        kartState.filter((item) => item.id !== selectedId)
      );
    }
  };
  const openKart = () => {
          ...
          <Html center portal={domContent}>
            ...
            <Card getKartData={getKartData} session={session} />
            {
              <Modal
                sz="xl"
                title={
                  <div className="tracking-wide uppercase font-medium text-left p-2">
            ...
            >
              <Timeline mode="alternate">
                ...
                <Timeline.Item>
                  Order Shipped to location 2015-09-01
                </Timeline.Item>
                <Timeline.Item color="green">
                  Order Recieved 2015-09-01
                </Timeline.Item>
              </Timeline>
            </Drawer>
          </Html>
